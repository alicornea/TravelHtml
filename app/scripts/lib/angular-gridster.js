          ...
          for (; index < length; index++) {
            ...
            transclude(childScope, function(clone) {
              ...
              if (layout) {
                ...
                clone.attr('data-num', layout.num = layout.num || -1);
                ...
                var html = clone.html();
                html = html.replace(/{link}/g,layout.link)
                clone.attr('data-link', layout.link);
                
                html = html.replace(/{txt}/g,layout.txt ? layout.txt : "")
                clone.attr('data-txt', layout.txt);
                
                html = html.replace(/{icon}/g,layout.icon)
                clone.attr('data-icon', layout.icon);
                
                
                html = html.replace(/{color}/g,layout.color)
                clone.attr('data-color', layout.color);
                ...
                if(layout.img)
                  html = html.replace(/{img}/g, "style='background:url(\""+layout.img+"\")'")
                else
                  html = html.replace(/{img}/g, "")
                  
                clone.attr('data-img', layout.img);
              ...
              var parentDiv = clone.find(".col");
              if (clone.width() < 200 && !parentDiv.hasClass("col-half-width")) {
              ...
              if (clone.height() < 60 && !parentDiv.hasClass("col-half-height")) {
                  parentDiv.addClass("col-half-height")
              }
    
              if (clone.width() > 60 && parentDiv.hasClass("col-half-height")) {
